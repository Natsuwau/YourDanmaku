...
<body>
	...
	<div class="x-body">
		<form class="layui-form x-center"
			action="<%=basePath%>/cms/frontVideo/checkList">
		...
		<table class="layui-table">
			...
			<c:choose>
				<c:when
					...
					<blockquote class="layui-elem-quote layui-quote-nm">待审核列表为空!</blockquote>
				</c:when>
				<c:otherwise>
					<tbody>
						<c:forEach items="${pageInfo.basePageList}" var="one">
							<tr title="${one.id}">
								...
								<td class="td-manage">
									...
									<a title="审核" href="javascript:;" onclick="change_state(this,'${one.id}','${one.state}')"
										style="text-decoration:none">
										<i class="layui-icon" >&#xe606;</i>
									</a>
								</td>
	...
	<script>
			...
			laypage({
			    ...
			    jump: function(obj, first){
				    ...
				    if(!first){
					    ...
					    var url = '<%=basePath%>/cms/frontVideo/checkList?nowPage='+nowPage;
		...
		function checkVideo(id,state) {
			var url ="<%=basePath%>cms/frontVideo/checkVideo";
		...
		function change_state(obj, id,state) {
			layer.confirm('<center>对这条记录的审核结果是?<center>',
					{btn:['通过','驳回','取消'],
				offset: '20%'
				},function(index) {
					checkVideo(id,"1");
