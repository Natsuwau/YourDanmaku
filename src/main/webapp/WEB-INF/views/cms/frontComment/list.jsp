...
<body>
	...
	<div class="x-body">
		<form class="layui-form x-center"
			action="<%=basePath%>/cms/frontComment/list">
		...
		<table class="layui-table">
			...
			<c:choose>
				<c:when
					...
					<blockquote class="layui-elem-quote layui-quote-nm">评论列表为空!</blockquote>
				</c:when>
				<c:otherwise>
					<tbody>
						<c:forEach items="${pageInfo.basePageList}" var="one">
							<tr title="${one.id}">
								<td><fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss" value="${one.time}" /></td>
								<td>
								...
								<td>${one.refId}</td>
								<td class="td-manage">
									<a title="查看管理评论" href="javascript:;"
	...
	<script>
			...
			laypage({
			    ...
			    jump: function(obj, first){
				    ...
				    if(!first){
					    ...
					    var url = '<%=basePath%>/cms/frontComment/list?nowPage='+nowPage;
		...
		function banVideo(id) {
			var url ="<%=basePath%>cms/frontComment/del";
